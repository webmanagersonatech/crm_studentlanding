...
export default function ForgotPasswordPage({ instituteId }: Props) {
  ...
  const handleSendOtp = async (e: FormEvent) => {
    ...
    const res = await sendStudentOtp({
      email,
      instituteId // ✅ now it's guaranteed string
    });
  ...
  const handleVerifyOtp = async (e: FormEvent) => {
    ...
    const res = await verifyStudentOtp({
      email,
      otp,
      instituteId // ✅ now TS knows it's string
    });
